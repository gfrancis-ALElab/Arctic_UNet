...
def sliding_window(image_width,image_height,
    ...
    Args:
        ...
        adj_overlay_x: the extended distance (in pixel of x direction)
        to adjacent patch, make each patch has overlay with adjacent patch
        adj_overlay_y: the extended distance (in pixel of y direction)
        to adjacent patch, make each patch has overlay with adjacent patch
...
def split_image(input,output_dir,patch_w=1024,patch_h=1024,adj_overlay_x=0,adj_overlay_y=0,out_format='PNG', pre_name = None):
    ...
    # print('Split finished at: %s'%str(finish))
    # print('Total run time: %s'%(finish-time))
